...
import { ApiResponse, Auth, NewAuth } from '@/modules/auth/@types/authTypes';
...
export async function registerUser(data: NewAuth): Promise<ApiResponse<Auth>> {
  try {
...
export async function loginUser({ email, password }: LoginPayload): Promise<ApiResponse<Auth>> {
  try {
...
export async function recoverPassword({
  email,
  newPassword,
}: RecoverPayload): Promise<ApiResponse<Auth>> {
  try {
    const user = await authRepo.findUserByEmail(email);
